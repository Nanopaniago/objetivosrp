...
/**
 * Gera um UUID válido mesmo em ambientes onde
...
export const GoalsManager: React.FC<GoalsManagerProps> = ({
  ...
  const workDaysCount =
    ...
    scheduleStats.totalMonthWorkDays > 0
      ? scheduleStats.totalMonthWorkDays
      : 22;
  ...
  const parsedMonthlyValues = useMemo(() => {
    ...
    categories.forEach(cat => {
      res[cat.slug] = parseVal(
        monthlyInputs[cat.slug]
      );
    });
  ...
  const handleSellerChange = (
    ...
    const updatedMonthly: Record<CategorySlug, string> =
      {} as any;
  ...
  const handleSave = (
    ...
    const otherGoals = goals.filter(
      g =>
        !(
          g.sellerId === selectedSellerId &&
          g.month === currentMonth &&
          g.year === currentYear
        )
    );

    const newGoalsForSeller: MonthlyGoal[] =
      categories.map(cat => {
        const mVal =
          parsedMonthlyValues[cat.slug] || 0;

        const dailyTarget =
          workDaysCount > 0 && mVal > 0
            ? Number(
                (
                  mVal / workDaysCount
                ).toFixed(2)
        ...
        const existingGoal = goals.find(
          item =>
            item.sellerId === selectedSellerId &&
            item.categorySlug === cat.slug &&
            item.month === currentMonth &&
            item.year === currentYear
        );
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-5">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            Defina a meta mensal de cada serviço.
            O objetivo diário é calculado
            automaticamente pela divisão do valor
            mensal pelos dias de trabalho na escala (
            {workDaysCount} dias).
          </p>
          ...
          <span className="text-slate-600">
            ...
            {scheduleStats.hasSchedule ? (
              ...
              <>
                ...
                <strong>
                  {scheduleStats.totalMonthWorkDays} dias
                  de trabalho
                </strong>{' '}
              ...
              <span className="text-amber-700 font-semibold">
                Sem escala personalizada
                (a utilizar 22 dias úteis padrão)
              </span>
      ...
      <form onSubmit={handleSave}>
                  ...
                  <div className="mt-1 flex items-center justify-between text-[10px] text-blue-800/80 pt-1 border-t border-blue-100">
                    ...
                    <span className="font-semibold">
                      ...
                      {mVal > 0
                        ...
                        : `Meta Mensal ÷ ${workDaysCount} dias`}
        ...
        <div className="mt-6 pt-4 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <span className="text-xs text-slate-500 text-center sm:text-left flex items-center gap-1.5">
            ...
            As metas definidas aplicam-se a{' '}
